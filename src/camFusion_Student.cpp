...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    cout << "Final Maches in Box: " << matches.size() << endl;
    ...
    cout << "BOUNDING Box: " << boundingBox.kptMatches.size() << endl;
}
...
{
    vector<cv::Point2f> matched_points1, matched_points2; // these are your points that match
    vector<int> prevframeIDX, preframeboxIDs, currframeIDX, currframeboxIDs;
    ...
    for (int i = 0; i < matches.size(); i++)
    {
        ...
        for (auto prevframeboxes = 0; prevframeboxes < prevFrame.boundingBoxes.size(); ++prevframeboxes) {
            if (prevFrame.boundingBoxes[prevframeboxes].roi.contains(P)) {
                ...
                preflag = true;
                preframeboxIDs.push_back(prevFrame.boundingBoxes[prevframeboxes].boxID);
        ...
        if ((preflag) && (currflag)) {
            bbBestMatches.insert(pair<int, int>(preframemap[idx2], currframemap[idx1]));
